          ...
          <table>
            ...
            <tbody>
              <tr v-for="item in recommendations" :key="item.id">
                ...
                <td>
                  <span :class="trendBadgeClass(item.trend)">{{ item.trend }}</span>
                </td>
  ...
  setup() {
    ...
    const recommendations = computed(() => {
      ...
      for (const item of sorted) {
        ...
        const qty = Math.min(Math.floor(remaining / item.unit_cost), item.forecasted_demand)
        if (qty <= 0) continue
    ...
    const totalCost = computed(() =>
      Math.round(recommendations.value.reduce((sum, r) => sum + r.item_total, 0) * 100) / 100
    )
    ...
    const placeOrder = async () => {
      ...
      try {
        ...
        orderError.value = t('restocking.orderError') + ' ' + (err.message || err)
      } finally {
